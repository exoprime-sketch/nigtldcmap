...
console.log("=== v101 operational finalization audit ===");
...
assert(
  serviceCatalog?.meta?.demoOnlyCount === 137,
  `service catalog pending ${serviceCatalog?.meta?.demoOnlyCount ?? 0}/137`
);
assert(
...
// 4. 공개 상세 경로에서 synthetic preview 제거
...
assert(
  ...
  "미연결 항목 공개 경로에 synthetic final preview가 남아 있음"
);
assert(
...
assert(
  ...
  "Dataset metadata-only fallback v101 누락"
);
assert(
  !renderer.includes("DatasetExamplePreviewV47"),
  "공개 Dataset fallback에서 예시 preview를 참조함"
);
const indexSource = read("src/index.tsx");
assert(
  indexSource.includes("runOperationalFinalizationAuditV101"),
  "개발 Preview가 v101 runtime audit를 실행하지 않음"
);
assert(
...
// 8. index.tsx 기준 실제 도달 가능한 공개 코드에서 예시 preview 컴포넌트 금지
const reachable = reachableFrom("src/index.tsx");
facts.reachableSourceFiles = reachable.size;
const bannedReachableImports = [
  "CountryDataFinalPreviewV53",
  "DatasetExamplePreviewV47",
];
for (const file of reachable) {
  if (!/[.](ts|tsx)$/.test(file)) continue;
  const text = fs.readFileSync(file, "utf8");
  for (const marker of bannedReachableImports) {
    if (text.includes(marker))
      failures.push(`${rel(file)} 공개 도달경로에서 ${marker} 참조`);
  }
}
...
const artifactDir = abs("artifacts/release-v101");
