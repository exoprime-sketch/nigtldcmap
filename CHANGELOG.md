...
- 지도: '핵심 레이어' 20개(`src/data/map/mapDefaultLayersV160.json`) 기본, 나머지 6분류는 '더 많은 레이어' 아래(`layers=all`로 펼침). map-index·빌더·렌더러 불변
...
- 공용 미니맵 `MiniMapV152`(홈 '주요 지역 데이터'·상세 작은 지도): 정적 SVG가 첫 화면·인쇄·대체 화면이고, 마우스를 올리거나(300ms)·포커스·터치·클릭·버튼으로 조작 의도를 보이면 MapLibre 엔진(동적 import 청크 `minimap-engine-v152`)으로 바뀜. `cooperativeGestures`(휠은 페이지 스크롤, Ctrl+휠 확대, 두 손가락 이동), [+]·[−]·[전체 보기]·[배경지도] 버튼, 키보드(화살표·+/−·Home·Esc), 확대 4~12, 이동 한계 베트남+2°, 한글 지명은 zoom 6 이상, 화면 밖이면 인스턴스 해제(페이지당 1개). 문서 `docs/MINIMAP_V152.md`
- 지도 아이콘 35종 `src/data/map/mapIconsV152.ts`(Tabler Icons MIT 31 · Material Symbols Apache-2.0 2 · 자체 제작 2, SVG path만 번들) — 15개 점 계열 레이어의 분류별 아이콘(A-023 발전원 12종·B-012 재해유형·C-025 등록제도·B-048 광종·E-005 기관유형 등). 문서 `docs/MAP_ICON_CONTRACT_V152.md`
- 레이어 렌더러 `src/map/layers/*`(RealMapExplorerPage에서 추출, 큰 지도·미니맵 공통 `renderMapLayerV152`)
- 검증 러너 `scripts/v152/`: 스타일 스냅샷 비교, 미니맵 조작 실측, 지도 아이콘, 6폭 넘침, 홈 LCP

### Changed

- 큰 지도 점 레이어: 흰 원 배지(테두리 = 분류색) + 아이콘, 클러스터에 대표 아이콘, hover 링, E-018·E-019 KR 표시. 범례는 아이콘+분류명+개수(지도와 같은 피처 속성으로 계산), A-023 설비용량 크기 범례를 실제 배지 크기 4구간과 일치
- 점 팝업·큰 지도 우측 패널: 규격 있는 9개 데이터는 라벨형 카드(국가·명칭·유형·소유·운영·규모·연도·소재지·자료 출처, 미기재 유지)
- 상세 작은 지도: 송전망 선 색을 큰 지도와 같은 전압 3구간으로, 점 60개 이하 레이어는 아이콘 배지, 선택 패널은 라벨형 카드. '큰 지도에서 비교'는 미니맵에서 움직인 위치·배율과 변수·기간을 URL(`view`·`mapSelectors`)로 넘김
- 이용안내 '지도 이용 시 참고사항'에 작은 지도 조작법·지도 기호 라이선스 문단
- 첫 로드: 정적 미니맵은 지점을 그리는 레이어일 때만 아이콘 묶음(동적 청크 `map-icon-kit-v152`)을 받고, 배경지도 판단·출처 문구는 엔진 청크 안에 둠 — 홈·선/면 상세의 첫 로드 JS 증가 gzip 약 +5.9 KB, maplibre·엔진은 조작 의도 전 0

### Fixed

- 시설 카드 숫자 칸: 원자료에 값이 없으면 `0`이 아니라 '미기재'로 표시(예: C-025 연간 예상감축 388건 중 262건이 '0 tCO₂e/년'으로 보이던 기존 결함 — 이번에 지도 팝업·선택 패널에도 카드를 쓰게 되어 함께 수정)
