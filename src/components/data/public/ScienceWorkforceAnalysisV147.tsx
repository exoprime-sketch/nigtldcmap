...
export default function ScienceWorkforceAnalysisV147({ rows }: { rows: SemanticObservationV125[] }) {
  ...
  return <section className="detail146" data-testid="science-workforce-v147">
    ...
    {comparable && <section className="d153-block" data-analysis-block="category-bar"><AnalysisBarsV147 title={`${selectedYear}년 · 고등교육 졸업자 중 STEM 전공 비중`} unit="%" maximum={100} rows={shares.map(([id, label]) => ({ id, label, value: value(id) }))} /></section>}
    <div className="detail146-table" data-analysis-block="table"><table><caption><PublicTermTextV134 text="STEM 졸업 비중 · UNESCO UIS" /></caption><thead><tr><th scope="col">구분</th><th scope="col">비중(%)</th><th scope="col">기준연도</th></tr></thead><tbody>{shares.map(([id, label]) => <tr key={id}><th scope="row">{label}</th><td>{formatValueV121(value(id))}</td><td>{own(id)[0]?.year || "미기재"}</td></tr>)}</tbody></table></div>
    <h3>연구자 규모</h3>
    <div className="detail146-table" data-analysis-block="table"><table><caption>절대수와 인구 대비 연구자 수를 구분합니다.</caption><thead><tr><th scope="col">항목</th><th scope="col">값</th><th scope="col">단위</th><th scope="col">연도</th></tr></thead><tbody>{[["researcher_count", "연구자 수(환산값)"], ["researchers_per_million", "인구 100만 명당 연구자 수"]].map(([id, label]) => <tr key={id}><th scope="row">{label}</th><td>{formatValueV121(value(id))}</td><td>{own(id)[0]?.unit}</td><td>{own(id)[0]?.year}</td></tr>)}</tbody></table></div>
