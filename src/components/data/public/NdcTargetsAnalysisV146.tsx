...
export default function NdcTargetsAnalysisV146({ entities }: { entities: VietnamEntityV124[] }) {
  ...
  return <section className="detail146" data-testid="ndc-targets-v146">
    <p>2022년 제출 NDC 기준입니다. 국제지원 조건부 목표는 자체 이행분을 포함한 전체 목표이며, 두 목표를 더하지 않습니다.</p>
    <div className="detail146-table"><table><caption>2030년 목표 · 2022년 NDC</caption>
      <thead><tr><th scope="col">항목</th><th scope="col">자체 이행</th><th scope="col">국제지원 시 전체</th><th scope="col">단위</th></tr></thead>
      <tbody>{model.totals.map((row) => <tr key={row.label}><th scope="row">{row.label}</th><td>{number(row.own)}</td><td>{number(row.supported)}</td><td>{row.unit}</td></tr>)}</tbody>
    </table></div>
    <p className="detail146-note">감축률은 추가 감축정책이 없는 경우의 2030년 배출 전망(BAU) 대비입니다. 소요재원은 감축목표 이행 비용이며, 확보된 재원이 아닙니다.</p>
    ...
    <figure className="detail146-chart"><figcaption>부문별 감축량 · {condition === "own" ? "자체 이행" : "국제지원 시 전체"} · 2030년 · 백만 tCO₂e</figcaption>
