...
export default function InfrastructureCoverageV147({ rows }: { rows: SemanticObservationV125[] }) {
  ...
  return <section className="detail146" data-testid="infrastructure-coverage-v147">
    ...
    {/* The count per data layer opens the screen (V153 contract: category-bar); the table restates it with the year. */}
    <section className="d153-block" data-analysis-block="category-bar"><AnalysisBarsV147 title="자료 종류별 수록 지물 수" unit="건" rows={groups.map((g) => ({ id: g.key, label: LAYERS[g.key] || g.row.displayLabel, value: g.row.value as number }))} /></section>
    <div className="detail146-table" data-analysis-block="table"><table><caption>자료 종류별 수록 지물 수</caption><thead><tr><th scope="col">자료 종류</th><th scope="col">수록 지물 수(건)</th><th scope="col">자료연도</th></tr></thead><tbody>{groups.map((g) => <tr key={g.key}><th scope="row">{LAYERS[g.key] || g.row.displayLabel}</th><td>{formatValueV121(g.row.value)}</td><td>{g.row.year}</td></tr>)}</tbody></table></div>
    ...
    {classes.length ? <><section className="d153-block" data-analysis-block="category-bar"><AnalysisBarsV147 title={`${LAYERS[group.key] || group.key} · 분류별 수록 건수`} unit="건" rows={classes.map((r) => ({ id: r.recordId, label: label(r), value: r.value as number }))} /></section><details className="detail146-details"><summary>표로 보기 · 분류별 수록 건수</summary><div className="detail146-table" data-analysis-block="table"><table><thead><tr><th scope="col">분류</th><th scope="col">건수</th><th scope="col">연도</th></tr></thead><tbody>{classes.map((r) => <tr key={r.recordId}><th scope="row">{label(r)}</th><td>{formatValueV121(r.value)}</td><td>{r.year}</td></tr>)}</tbody></table></div></details></> : <p className="detail146-note">이 자료에는 분류별 건수가 수록되어 있지 않습니다.</p>}
