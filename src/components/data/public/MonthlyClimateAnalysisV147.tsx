...
export default function MonthlyClimateAnalysisV147({ rows }: { rows: SemanticObservationV125[] }) {
  ...
  return <section className="detail146" data-testid="monthly-climate-v147">
    ...
    {months.every((m) => m.temperature !== null) && <InteractiveTimeSeriesChartV127 title="월별 평년 기온" ariaLabel="1월부터 12월까지 평년 기온" unit="℃" xAxisTitle="월" yAxisTitle="평년 기온" formatValue={formatValueV121} minimumVisibleSeries={1} series={[{ id: "temperature", label: "월평균 기온", unit: "℃", points: months.map((m) => ({ id: `month-${m.month}`, x: m.month, xLabel: `${m.month}월`, value: m.temperature as number })) }]} />}
    <p className="detail146-note">건기·우기는 원자료의 전국 분류를 표시했습니다. 원자료는 월강수량과 기준 월평균강수량(147.8 mm)을 비교해 분류합니다.</p>
    <details className="detail146-details"><summary>표로 보기 · 12개월</summary><div className="detail146-table"><table>
