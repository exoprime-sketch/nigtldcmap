...
export default function NationalResourceSeriesV147({ entities }: { entities: VietnamEntityV124[] }) {
  ...
  if (own.unitConflict) return <section className="detail146 analysis147-block" data-analysis-block="status-note" data-testid="national-resource-series-v147">
  ...
  return <section className="detail146 analysis147-block" data-testid="national-resource-series-v147">
    ...
    <details className="detail146-details" data-analysis-block="table" key={`table-${own.key}`} open={numeric.length < 2}><summary>표로 보기 · {own.label} · {own.points.length}개 연도</summary><div className="detail146-table"><table data-testid="national-series-table-v147"><caption><PublicTermTextV134 text={`${own.label} · 단위: ${own.unit} · 전국`} /></caption><thead><tr><th scope="col">연도</th><th scope="col">값</th><th scope="col">자료 기준</th></tr></thead><tbody>{own.points.map((p) => <tr key={p.year}><th scope="row">{p.year}</th><td>{p.conflict ? "동일 연도 중복 · 비교 제외" : p.value === null ? "자료 없음" : formatValueV121(p.value)}</td><td>{(publicTextV126(p.note) || "").replace(/^전국\s*·\s*/u, "") || "원자료 표기 없음"}</td></tr>)}</tbody></table></div></details>
  </section>;
}
