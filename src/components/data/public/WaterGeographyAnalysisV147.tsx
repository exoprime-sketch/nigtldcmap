...
export function FlowDirectionAnalysisV147({ entities }: { entities: VietnamEntityV124[] }) {
  ...
  return <section className="detail146" data-testid="flow-direction-v147">
    <h3>성·시별 물이 흐르는 방향</h3><p className="detail146-note">지형 격자에서 물이 흘러가는 방향의 비율입니다. 하천의 유량이나 강수량은 아닙니다. 원자료의 63개 성·시 경계를 기준으로 합니다.</p>
    ...
    <div className="detail146-table"><table><caption>{regionLabel(own)} · HydroSHEDS DIR 15s · 비율 합계는 반올림으로 100%와 다를 수 있습니다.</caption><thead><tr><th scope="col">방향</th><th scope="col">비율(%)</th></tr></thead><tbody>{values.map((v) => <tr key={v.id}><th scope="row">{v.label}</th><td>{formatValueV121(v.value)}</td></tr>)}</tbody></table></div>
  </section>;
}
...
export function BasinAreaAnalysisV147({ entities }: { entities: VietnamEntityV124[] }) {
  ...
  return <section className="detail146" data-testid="basin-area-v147">
    <h3>유역 전체 면적과 베트남 내 면적</h3><p className="detail146-note">국경을 넘는 유역 전체와 베트남에 속한 면적을 구분합니다. 문헌 값과 GIS 산출값은 계산 기준이 달라 별도로 제공합니다.</p>
