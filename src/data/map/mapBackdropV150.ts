...
// Names follow the 63-unit boundary vintage, not the post-2025 34-unit map.
...
export function addKoreanMapLabelsV150(map: MapLibreMap, boundary: VietnamMapGeoJsonV124 | null) {
  ...
  for (const f of boundary?.features || []) {
    const name = PROVINCE_KO_V150[String(f.properties.adm1Code)]; const point = polygonLabelPointV150(f.geometry);
