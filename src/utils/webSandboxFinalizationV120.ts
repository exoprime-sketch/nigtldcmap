...
} from "../data/cooperation/publicDetailCopyV120";
...
export interface WebSandboxQaResultV120 {
  schemaVersion: "v120";
  generatedAt: string;
  ...
  facts: {
    ...
    publicDetailCopy: typeof PUBLIC_DETAIL_COPY_SUMMARY_V120 & {
      uniqueIds: number;
    ...
    runtimePolicy: typeof PUBLIC_DETAIL_RUNTIME_POLICY_V120;
  };
}
...
export async function runWebSandboxFinalizationV120(
  onProgress?: (message: string) => void
): Promise<WebSandboxQaResultV120> {
  ...
  const ids = PUBLIC_DETAIL_COPY_DEFINITIONS_V120.map((item) => item.elementId);
  const uniqueIds = new Set(ids);
  const missingQuestions = PUBLIC_DETAIL_COPY_DEFINITIONS_V120.filter(
    (item) => !item.publicQuestion.trim() || item.supportFields.length === 0
  );
  const unsupportedSpatialQuestions =
    PUBLIC_DETAIL_COPY_DEFINITIONS_V120.filter(
      (item) =>
        SPATIAL_LANGUAGE.test(item.publicQuestion) &&
        !contractHasDetailedSpatialField(item.elementId)
    );
  const invalidMapActions = PUBLIC_DETAIL_COPY_DEFINITIONS_V120.filter(
    (item) => {
  ...
  const plannedDownloadActions = PUBLIC_DETAIL_COPY_DEFINITIONS_V120.filter(
    (item) => {
      const contract = DATA_DISPLAY_CONTRACT_INDEX_V118.get(item.elementId);
      return (
  ...
  const visibleCopyText = [
    ...PUBLIC_DETAIL_COMMON_UI_COPY_V120,
    ...PUBLIC_DETAIL_COPY_DEFINITIONS_V120.flatMap((item) => [
      item.publicQuestion,
      item.compactUseNote ?? "",
      item.compactCaution ?? "",
      item.analysisTitle,
  ...
  const bannedTermsInPublicCopy =
    PUBLIC_DETAIL_BANNED_VISIBLE_TERMS_V120.filter((term) =>
  ...
  addCheck(
    ...
    "152개 공개 상세문구 정의",
    PUBLIC_DETAIL_COPY_DEFINITIONS_V120.length === 152 &&
      uniqueIds.size === 152,
    `${PUBLIC_DETAIL_COPY_DEFINITIONS_V120.length}/152 · 고유 ID ${uniqueIds.size}`,
    "152/152 · 고유 ID 152"
  );
  addCheck(
    checks,
  ...
  const auditPolicyPass = PUBLIC_DETAIL_COPY_DEFINITIONS_V120.every(
    (item) =>
  ...
  addCheck(
    ...
    "대형 역할·흐름·공간카드 제거정책",
    auditPolicyPass &&
      PUBLIC_DETAIL_RUNTIME_POLICY_V120.largeGenericGuidanceBeforeData ===
        false &&
      PUBLIC_DETAIL_RUNTIME_POLICY_V120.independentSpatialCardPublic === false,
    `정책 ${String(auditPolicyPass)} · 대형안내 ${String(
      PUBLIC_DETAIL_RUNTIME_POLICY_V120.largeGenericGuidanceBeforeData
    )} · 공간카드 ${String(
      PUBLIC_DETAIL_RUNTIME_POLICY_V120.independentSpatialCardPublic
    )}`,
    "true · false · false"
  );
  addCheck(
    ...
    PUBLIC_DETAIL_RUNTIME_POLICY_V120.plannedSyntheticValuesPublic === false &&
      PUBLIC_DETAIL_RUNTIME_POLICY_V120.plannedSyntheticRankingPublic ===
        false &&
      PUBLIC_DETAIL_RUNTIME_POLICY_V120.plannedDirectDownloadPublic === false,
    `값 ${String(
      PUBLIC_DETAIL_RUNTIME_POLICY_V120.plannedSyntheticValuesPublic
    )} · 순위 ${String(
      PUBLIC_DETAIL_RUNTIME_POLICY_V120.plannedSyntheticRankingPublic
    )} · 다운로드 ${String(
      PUBLIC_DETAIL_RUNTIME_POLICY_V120.plannedDirectDownloadPublic
    )}`,
    "false · false · false"
  );
  addCheck(
    ...
    PUBLIC_DETAIL_RUNTIME_POLICY_V120.relatedDataUsesExistingElementRoute &&
      PUBLIC_DETAIL_RUNTIME_POLICY_V120.downloadUsesDedicatedHub,
    `관련 route ${String(
      PUBLIC_DETAIL_RUNTIME_POLICY_V120.relatedDataUsesExistingElementRoute
    )} · 다운로드 허브 ${String(
      PUBLIC_DETAIL_RUNTIME_POLICY_V120.downloadUsesDedicatedHub
  ...
  const overall: WebSandboxQaResultV120["overall"] =
    p0 > 0 ? "BLOCKED" : p1 > 0 ? "CONDITIONALLY_READY" : "READY";

  const result: WebSandboxQaResultV120 = {
    schemaVersion: "v120",
    ...
    facts: {
      ...
      runtimePolicy: PUBLIC_DETAIL_RUNTIME_POLICY_V120,
    },
  };
  ...
  if (typeof window !== "undefined") {
    (
      window as typeof window & {
        __LDC_WEB_SANDBOX_QA_V119__?: WebSandboxQaResultV120;
      }
...
function overallLabel(overall: WebSandboxQaResultV120["overall"]): string {
...
function buildQaHtml(
  result: WebSandboxQaResultV120 | null,
  progress: string,
  error: string | null
): string {
...
function downloadResult(result: WebSandboxQaResultV120): void {
  ...
  a.href = url;
  a.download = `ldc-web-sandbox-qa-v120-${new Date()
    .toISOString()
    .replace(/:/g, "-")}.json`;
...
export function isWebSandboxQaRequestedV120(): boolean {
  return new URLSearchParams(window.location.search).get("qa") === "1";
}

export async function mountWebSandboxQaV120(root: HTMLElement): Promise<void> {
  let currentResult: WebSandboxQaResultV120 | null = null;
  let running = false;
  const render = (
  ...
  const execute = async () => {
    ...
    render("검사를 시작합니다");
    try {
      currentResult = await runWebSandboxFinalizationV120((message) =>
        render(message)
      );
    } catch (error) {
      render(
...
export const WEB_SANDBOX_V120_FACTS = {
  primaryInformationLabel: "주요 정보",
  categoryCount: 10,
  categoryContractsPresent: true,
  comparisonTemplates: [
    "indicator",
    "technology-demand",
    "policy",
  ...
  noCompositeCooperationScore: true,
  noSyntheticCountryRanking: true,
  noMissingCoordinateProjectPoints: true,
  noCrossFundFinancialAggregation: true,
  comparisonDownloadHubContext: true,
} as const;
