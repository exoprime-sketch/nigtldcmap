...
| 2026-09-24 | `scripts/audit-vietnam-temporal-depth-v135.mjs` GHG_ANALYTICAL_VIEW(C-002) | 부문 합계 막대 `figure li` 4개 | `figure li` 4개 **또는** 타일 목록(`reported-inventory-tiles-v159`) 4개 | PR #32 결정("⑥ 화면은 숫자 차트를 그리지 않음")으로 C-002 부문 합계 4개가 막대→타일로 바뀜. #32는 이 변경 뒤 게이트를 다시 돌리지 않아 V160 게이트 2회차에서 드러남. 표(4행×5열)·값 4개 검사는 불변 |
